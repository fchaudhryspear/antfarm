...
REGRESSION_TESTS: <test names added>
```
...
```
STATUS: skipped
REASON: no security findings in provided input
```
...
5. Run `{{ build_cmd }}` — must pass
6. Run `{{ test_cmd }}` — must pass
7. `git commit -m "fix(security): <description>"`
8. Repeat for next finding (max {{ max_fixes_per_domain }} fixes)
...
REGRESSION_TESTS: <test names / files added>
