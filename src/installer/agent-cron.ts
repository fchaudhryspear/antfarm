...
export async function teardownWorkflowCronsIfIdle(workflowId: string): Promise<void> {
  ...
  // Check ALL active runs across ALL workflows, not just this one
...
/**
 * Disable (pause) all crons for a workflow when a run reaches terminal state.
 * Unlike teardown which deletes, this only disables — crons can be re-enabled via resumeWorkflowCrons.
 * This prevents wasted polling cycles on completed/failed runs.
 */
export async function pauseWorkflowCrons(workflowId: string, reason: string): Promise<void> {
  const prefix = `antfarm/${workflowId}/`;
  const result = await listCronJobs();
  if (!result.ok || !result.jobs) {
    log.warn(`pauseWorkflowCrons: failed to list crons for ${workflowId}`);
    return;
  }

  const matching = result.jobs.filter((j: { name: string; enabled?: boolean }) => j.name.startsWith(prefix));
  if (matching.length === 0) {
    log.debug(`pauseWorkflowCrons: no crons found for ${workflowId}`);
    return;
  }
...
/**
 * Re-enable all crons for a workflow when a new run starts.
 */
export async function resumeWorkflowCrons(workflowId: string): Promise<void> {
  const prefix = `antfarm/${workflowId}/`;
  const result = await listCronJobs();
  if (!result.ok || !result.jobs) {
    log.warn(`resumeWorkflowCrons: failed to list crons for ${workflowId}`);
    return;
  }

  const matching = result.jobs.filter((j: { name: string; enabled?: boolean }) => j.name.startsWith(prefix) && j.enabled === false);
  if (matching.length === 0) {
    log.debug(`resumeWorkflowCrons: no disabled crons found for ${workflowId}`);
    return;
  }
